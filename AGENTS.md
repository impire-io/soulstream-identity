...
1. `../soul-hq/00-GENESIS/` — [`vision.md`](../soul-hq/00-GENESIS/vision.md) (the identity
...
4. `../soul-hq/02-DESIGN/soulstream-identity/` — the design docs and their numbered decisions (D1–D13
   ...
   **persona** is the represented subject, **principal** the server-proven
   (account, user) — "identity" is never a noun for a subject (D27).
...
- **Quality gate before "done"** (all green, none skipped, before every
  ...
  (go test ./...) includes the operator-mode end-to-end proof. The hq
...
- **Research** runs through `/research-start` → investigate →
  `/research-graduate` (`../soul-hq/01-RESEARCH/`).
- **Feature work is spec-driven via speckit**: `/speckit-specify` →
  ...
  projection of `../soul-hq/00-GENESIS/` — on conflict, hq wins. The hq duties below
